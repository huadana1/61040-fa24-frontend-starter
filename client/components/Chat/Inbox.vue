...
async function getChats() {
  try {
    ...
    const requestsSent: Array<string> = (await fetchy(`/api/friend/requests/from`, "GET", {})).map((record: any) => {
      return record.to;
    });
    const requestsReceived: Array<string> = (await fetchy(`/api/friend/requests/to`, "GET", {})).map((record: any) => {
      return record.from;
    });

    // nonActionablePendingInbox.value = inbox.filter(requestsSent.includes(getOtherUser(chat)));
    actionablePendingInbox.value = inbox;
    console.log(actionablePendingInbox.value);
...
<template>
  <section class="allChats" v-if="loaded">
    ...
    <section class="actionableChats">
      ...
      <ActionableChat v-for="chat in actionablePendingInbox" @click="openSelectedChat(chat)" :user2="getOtherUser(chat)" :class="selectedChat == getOtherUser(chat) ? selected : notSelected">
      </ActionableChat>
